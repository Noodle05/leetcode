...
public class RedBlackTree<K extends Comparable<? super K>, V> {
    ...
    /**
     * <p>
     * <b>Case 1:</b> The current node N is at the root of the tree. In this
     * case, it is repainted black to satisfy property 2 (the root is black).
     * Since this adds one black node to every path at once, property 5 (all
     * paths from any given node to its leaf nodes contain the same number of
     * black nodes) is not violated.
     * </p>
     * 
     * @param n
     */
     ...
     * children of every red node are black) is not invalidated. In this case,
     * the tree is still valid. Property 5 (all paths from any given node to its
     * leaf nodes contain the same number of black nodes) is not threatened,
     * because the current node N has two black leaf children, but because N is
     * red, the paths through each of its children have the same number of black
     * nodes as the path through the leaf it replaced, which was black, and so
     * this property remains satisfied.
     * </p>
     * 
     * @param n
     */
     ...
     * contain the same number of black nodes)). Now, the current red node N has
     * a black parent. Since any path through the parent or uncle must pass
     * through the grandparent, the number of black nodes on these paths has not
     * changed. However, the grandparent G may now violate properties 2 (The
     * root is black) or 4 (Both children of every red node are black) (property
     * 4 possibly being violated since G may have a red parent). To fix this,
     * the entire procedure is recursively performed on G from case 1. Note that
     * this is a tail-recursive call, so it could be rewritten as a loop; since
     * this is the only loop, and any rotations occur after this loop, this
     * proves that a constant number of rotations occur.
     * </p>
     * 
     * @param n
     */
     ...
     * current node N is the right child of P, and P in turn is the left child
     * of its parent G. In this case, a left rotation on P that switches the
     * roles of the current node N and its parent P can be performed; then, the
     * former parent node P is dealt with using case 5 (relabeling N and P)
     * because property 4 (both children of every red node are black) is still
     * violated. The rotation causes some paths (those in the sub-tree labelled
     * "1") to pass through the node N where they did not before. It also causes
     * some paths (those in the sub-tree labelled "3") not to pass through the
     * node P where they did before. However, both of these nodes are red, so
     ...
     * node N is the left child of P, and P is the left child of its parent G.
     * In this case, a right rotation on G is performed; the result is a tree
     ...
     * child P could not have been red otherwise (without violating property 4).
     * Then, the colors of P and G are switched, and the resulting tree
     * satisfies property 4 (both children of every red node are black).
     * Property 5 (all paths from any given node to its leaf nodes contain the
     * same number of black nodes) also remains satisfied, since all paths that
     * went through any of these three nodes went through G before, and now they
     * all go through P. In each case, this is the only black node of the three.
     * </p>
     * 
     * @param n
     */
    ...
    /**
     * <p>
     * <b>Case 1:</b> N is the new root. In this case, we are done. We removed
     * one black node from every path, and the new root is black, so the
    ...
    /**
     * <p>
     * <b>Case 2:</b> S is red. In this case we reverse the colors of P and S,
     * and then rotate left at P, turning S into N's grandparent. Note that P
     * has to be black as it had a red child. Although all paths still have the
     * same number of black nodes, now N has a black sibling and a red parent,
     * so we can proceed to step 4, 5, or 6. (Its new sibling is black because
     * it was once the child of the red S.) In later cases, we will relabel N's
     * new sibling as S.
     * </p>
     * 
     * @param n
    ...
    /**
     * <p>
     * <b>Case 4:</b> S and S's children are black, but P is red. In this case,
     * we simply exchange the colors of S and P. This does not affect the number
     * of black nodes on paths going through S, but it does add one to the
     * number of black nodes on paths going through N, making up for the deleted
    ...
    /**
     * <p>
     * <b>Case 5:</b> S is black, S's left child is red, S's right child is
     * black, and N is the left child of its parent. In this case we rotate
     * right at S, so that S's left child becomes S's parent and N's new
     * sibling. We then exchange the colors of S and its new parent. All paths
     * still have the same number of black nodes, but now N has a black sibling
     * whose right child is red, so we fall into case 6. Neither N nor its
     * parent are affected by this transformation. (Again, for case 6, we
    ...
    /**
     * <p>
     * <b>Case 6:</b> S is black, S's right child is red, and N is the left
     * child of its parent P. In this case we rotate left at P, so that S
     * becomes the parent of P and S's right child. We then exchange the colors
     * of P and S, and make S's right child black. The subtree still has the
     * same color at its root, so Properties 4 (Both children of every red node
     * are black) and 5 (All paths from any given node to its leaf nodes contain
     * the same number of black nodes) are not violated. However, N now has one
     ...
     * possibilities:
     * <ul>
     * <li>It goes through N's new sibling. Then, it must go through S and P,
     ...
     * <li>It goes through N's new uncle, S's right child. Then, it formerly
     * went through S, S's parent, and S's right child (which was red), but now
     ...
     * red or black, but must refer to the same color both before and after the
     * transformation.
     * </p>
     * 
     * @param n
     ...
     * Since the root can always be changed from red to black, but not
